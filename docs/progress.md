...
### ✅ 3.6 实现营养成分计算工具函数（已完成）

**完成时间**：2026年1月13日

**完成内容**：
1. 创建了营养成分计算工具模块（`utils/nutrition-calculator.ts`）
2. 实现了 `estimateNutrition` 函数，根据菜品名称和分量估算营养成分
3. 实现了基于关键词匹配的规则系统
4. 提供了默认营养成分值，用于未知菜品
5. 实现了根据分量缩放营养成分的功能
...
- `server/src/utils/` - 工具函数目录
  - `nutrition-calculator.ts` - 营养成分计算工具模块

**estimateNutrition 函数详情**：
- **参数**：
  - `dishName: string` - 菜品名称
  - `portion: number` - 分量（克），默认 200g（一份）
- **返回值**：`Nutrition` - 估算的营养成分（包含 calories, protein, fat, carbs）
- **功能流程**：
  1. 规范化菜品名称（转换为小写并去除空格）
  2. 遍历营养成分规则，查找匹配的关键词
  3. 如果找到匹配的规则，使用对应的营养成分
  4. 如果没有匹配的规则，使用默认营养成分值
  5. 根据分量缩放营养成分（默认每100g的营养成分）
  6. 返回缩放后的营养成分

**营养成分规则系统**：
- **高蛋白类**（肉类、蛋类）：
  - 鸡、鸡肉、鸡丁等：200 kcal/100g, 25g 蛋白质, 8g 脂肪
  - 猪、猪肉、排骨等：300 kcal/100g, 20g 蛋白质, 25g 脂肪
  - 牛、牛肉、牛排等：250 kcal/100g, 26g 蛋白质, 15g 脂肪
  - 鱼、鱼肉、鱼片等：150 kcal/100g, 20g 蛋白质, 5g 脂肪
  - 虾、虾仁等：100 kcal/100g, 18g 蛋白质, 1g 脂肪
  - 蛋、鸡蛋等：150 kcal/100g, 13g 蛋白质, 10g 脂肪
- **蔬菜类**（低卡路里）：
  - 青菜、白菜、菠菜等：20 kcal/100g, 2g 蛋白质, 0g 脂肪
  - 番茄、西红柿、茄子等：15 kcal/100g, 1g 蛋白质, 0g 脂肪
  - 豆、豆腐、豆芽等：80 kcal/100g, 8g 蛋白质, 3g 脂肪
- **主食类**（高碳水化合物）：
  - 饭、米饭、炒饭等：130 kcal/100g, 3g 蛋白质, 0g 脂肪, 28g 碳水化合物
  - 面、面条、拉面等：110 kcal/100g, 4g 蛋白质, 1g 脂肪, 22g 碳水化合物
  - 包、包子、饺子等：200 kcal/100g, 8g 蛋白质, 5g 脂肪, 35g 碳水化合物
- **高脂肪类**：
  - 炸、油炸、炸鸡等：350 kcal/100g, 15g 蛋白质, 25g 脂肪, 15g 碳水化合物
  - 烤、烧烤、烤肉等：250 kcal/100g, 20g 蛋白质, 15g 脂肪, 5g 碳水化合物
- **汤类**（低卡路里）：
  - 汤、鸡汤、鱼汤等：30 kcal/100g, 3g 蛋白质, 1g 脂肪, 2g 碳水化合物

**默认营养成分值**：
- 用于未知菜品或作为基础值
- 每100g：150 kcal, 10g 蛋白质, 5g 脂肪, 20g 碳水化合物
- 提供合理的默认值，确保所有菜品都能获得营养成分估算

**分量缩放功能**：
- `scaleNutrition` 函数根据实际分量（克）自动计算营养成分
- 计算公式：`营养成分 = 每100g的营养成分 × (分量 / 100)`
- 卡路里四舍五入到整数
- 蛋白质、脂肪、碳水化合物保留一位小数

**技术细节**：
- 使用关键词匹配算法，支持部分匹配（如"鸡"匹配"鸡肉"、"鸡丁"等）
- 规则按优先级顺序匹配，找到第一个匹配的规则即使用
- 支持中文菜品名称的关键词匹配
- 使用 TypeScript 接口确保类型安全
- 函数为纯函数，无副作用，便于测试
...
**使用示例**：
```typescript
import { estimateNutrition } from '../utils/nutrition-calculator';

// 估算"宫保鸡丁"的营养成分（默认200g）
const nutrition = estimateNutrition('宫保鸡丁');
// 返回：{ calories: 400, protein: 50, fat: 16, carbs: 0 }

// 估算"番茄鸡蛋"的营养成分（300g）
const nutrition2 = estimateNutrition('番茄鸡蛋', 300);
// 返回：根据匹配的规则计算
```

**验证结果**：
- ✅ 创建了营养成分计算工具模块
- ✅ 实现了 `estimateNutrition` 函数，支持根据菜品名称和分量估算营养成分
- ✅ 实现了基于关键词匹配的规则系统，包含多种常见食材和菜品类型
- ✅ 提供了默认营养成分值，用于未知菜品
- ✅ 实现了根据分量缩放营养成分的功能
- ✅ 输入不同菜品名称，返回合理的营养成分估算
- ✅ 营养成分值在合理范围内
- ✅ 函数能处理未知菜品（返回默认值）
...
**完成内容**：
1. 创建了数据库种子脚本（`database/seeds/seed-dishes.ts`）
2. 准备了 30 个常见菜品数据，包含完整的营养成分数据
3. 实现了脚本的幂等性（可重复运行，不会产生重复数据）
4. 在 `package.json` 中添加了 `seed:dishes` 命令

**创建的文件和目录**：
- `server/src/database/seeds/` - 种子数据脚本目录
  - `seed-dishes.ts` - 菜品种子数据脚本

**修改的文件**：
- `server/package.json` - 添加了 `seed:dishes` 命令

**种子数据内容**：
- **总计**：30 个常见菜品
- **分类分布**：
  - 川菜：4 个（宫保鸡丁、麻婆豆腐、回锅肉、水煮鱼）
  - 粤菜：4 个（白切鸡、叉烧包、清蒸鲈鱼、白灼虾）
  - 湘菜：2 个（剁椒鱼头、小炒肉）
  - 鲁菜：2 个（糖醋里脊、红烧肉）
  - 主食：4 个（蛋炒饭、牛肉面、小笼包、饺子）
  - 汤类：3 个（西红柿鸡蛋汤、冬瓜排骨汤、紫菜蛋花汤）
  - 素菜：3 个（蒜蓉西兰花、地三鲜、麻婆豆腐（素））
  - 海鲜：2 个（清蒸大闸蟹、蒜蓉粉丝蒸扇贝）
  - 日式：2 个（寿司、拉面）
  - 西式：3 个（意大利面、牛排、沙拉）
  - 韩式：2 个（石锅拌饭、泡菜汤）
...
**每个菜品包含的完整数据**：
- `name` - 菜品名称（必填）
- `category` - 菜品分类（如：川菜、粤菜、主食等）
- `cuisineType` - 菜系类型（如：中式、日式、西式、韩式）
- `nutrition` - 营养成分对象（包含 calories, protein, fat, carbs）
- `tags` - 标签数组（如：['辣', '下饭']、['清淡', '健康']）
- `description` - 菜品描述

**种子脚本功能**：
- **幂等性**：通过检查菜品名称是否存在来避免重复插入
  - 如果菜品已存在，跳过并记录日志
  - 如果菜品不存在，创建新记录
- **错误处理**：包含完整的错误处理和日志输出
- **连接管理**：自动管理数据库连接的建立和关闭
- **统计信息**：显示插入和跳过的菜品数量

**seedDishes 函数实现详情**：
- **功能流程**：
  1. 初始化数据源连接（如果未初始化）
  2. 获取 Dish 实体的 Repository
  3. 遍历种子数据数组
  4. 对每个菜品：
     - 检查是否已存在（根据名称）
     - 如果存在，跳过并记录日志
...
**package.json 脚本命令**：
- `seed:dishes` - 运行菜品种子数据脚本
  - 命令：`ts-node -r tsconfig-paths/register src/database/seeds/seed-dishes.ts`
  - 使用 `ts-node` 直接运行 TypeScript 文件
  - 使用 `tsconfig-paths/register` 支持路径别名

**使用方法**：
```bash
cd server
pnpm run seed:dishes
```
...
**技术细节**：
- 使用 TypeORM 的 DataSource 连接数据库
- 使用 Repository 进行数据库操作
- 使用 `findOne()` 检查菜品是否存在
- 使用 `create()` 和 `save()` 创建和保存菜品
- 支持直接运行脚本（`require.main === module`）
- 支持作为模块导入（`export { seedDishes }`）

**验证结果**：
- ✅ 创建了数据库种子脚本
- ✅ 准备了 30 个常见菜品数据，包含完整的营养成分数据
- ✅ 实现了脚本的幂等性（可重复运行）
- ✅ 在 `package.json` 中添加了 `seed:dishes` 命令
- ⏳ 运行种子脚本，应成功插入数据（需要用户验证）
- ⏳ 多次运行脚本，不应产生重复数据（需要用户验证）
- ⏳ 检查数据库，应包含所有初始菜品（需要用户验证）
- ⏳ 每个菜品应包含完整的营养成分（需要用户验证）

**下一步**：3.8 实现随机推荐菜品 API
...
### ✅ 3.11 实现推荐页面 UI（含骨架屏和加载动画）（已完成）

**完成时间**：2026年1月13日

**完成内容**：
1. 在 `services/dishes.ts` 中添加了 `getRandomDish()` 函数，调用后端 `/v1/dishes/recommend/random` 端点
2. 创建了推荐页面组件 `app/recommend.tsx`，包含：
   - 使用 React Query 获取随机推荐菜品
   - 骨架屏组件：数据加载时显示占位符
   - 加载动画：按钮点击时显示加载状态
   - "换一个"按钮：重新获取推荐
   - 错误处理：显示友好的错误信息
   - 菜品信息展示：名称、分类、菜系、描述、标签、营养成分

**创建/修改的文件**：
- `mobile/services/dishes.ts` - 添加了 `getRandomDish()` 函数
- `mobile/app/recommend.tsx` - 新建推荐页面组件

**services/dishes.ts 更新**：
- **getRandomDish() 函数**：
...
**功能特性**：
1. **骨架屏**：加载时显示占位符，避免空白页面
2. **加载动画**：按钮点击时显示"获取中..."和加载指示器
3. **错误处理**：网络错误或数据错误时显示友好提示
4. **菜品展示**：完整展示菜品信息（名称、分类、菜系、描述、标签、营养成分）
5. **路由守卫**：未认证时自动重定向到登录页
6. **UI 风格**：与项目其他页面保持一致

**使用方式**：
- 访问 `/recommend` 路由即可查看随机推荐的菜品
- 点击"换一个"按钮可以获取新的推荐
- 页面会自动处理加载状态和错误状态

**验证结果**：
- ✅ 创建了 `services/dishes.ts` 中的 `getRandomDish()` 函数
- ✅ 创建了 `app/recommend.tsx` 推荐页面组件
- ✅ 实现了骨架屏组件，数据加载时显示占位符
- ✅ 实现了加载动画，按钮点击时显示加载状态
- ✅ 实现了"换一个"按钮，支持重新获取推荐
- ✅ 实现了错误处理，显示友好的错误信息
- ✅ 实现了菜品信息展示（名称、分类、菜系、描述、标签、营养成分）
- ✅ 应用了路由守卫，未认证时自动重定向
- ✅ TypeScript 编译通过，无类型错误
- ⏳ 访问 `/recommend` 路由，页面应正常显示（需要用户验证）
- ⏳ 页面应显示菜品信息（名称、描述、营养成分等）（需要用户验证）
- ⏳ 点击"换一个"按钮，应获取新的推荐（需要用户验证）
- ⏳ 加载状态应显示骨架屏（不是空白）（需要用户验证）
- ⏳ 按钮点击时应显示加载动画（"获取中..."）（需要用户验证）
- ⏳ 错误状态应显示友好的错误信息（需要用户验证）
- ⏳ 页面首次加载时间应小于 1.5 秒（需要用户验证）

**下一步**：3.12 创建饮食记录实体（DietRecord Entity）
