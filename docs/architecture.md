...
### 种子数据目录（src/database/seeds/）

#### `database/seeds/`
- **作用**：存放数据库种子数据脚本
- **位置**：`src/database/seeds/`
- **用途**：
  - 初始化数据库的初始数据
  - 填充测试数据
  - 为开发环境提供示例数据
...
#### `database/seeds/seed-dishes.ts`
- **作用**：菜品种子数据脚本，用于初始化菜品数据
- **功能**：
  - 插入 30 个常见菜品到数据库
  - 每个菜品包含完整的营养成分数据
  - 支持幂等性运行（多次运行不会产生重复数据）
- **种子数据内容**：
  - **总计**：30 个常见菜品
  - **分类分布**：
...
- **核心函数**：
  - `seedDishes()`: 执行种子数据插入
    ...
    - 关闭数据库连接
- **幂等性实现**：
  - 使用 `dishRepository.findOne({ where: { name: dishData.name } })` 检查菜品是否存在
  - 如果菜品已存在，跳过并记录日志
  - 如果菜品不存在，创建新记录
  - 确保多次运行脚本不会产生重复数据
- **错误处理**：
  - 使用 try-catch 捕获错误
  - 输出详细的错误信息
  - 确保数据库连接正确关闭（使用 finally 块）
- **连接管理**：
  - 使用 `AppDataSource.initialize()` 建立连接
  - 使用 `AppDataSource.destroy()` 关闭连接
  - 检查连接状态（`AppDataSource.isInitialized`）
- **运行方式**：
  - **直接运行**：`pnpm run seed:dishes`
    - 使用 `require.main === module` 判断是否直接运行
    - 执行完成后退出进程（`process.exit()`）
  - **作为模块导入**：`import { seedDishes } from './seeds/seed-dishes'`
    - 导出 `seedDishes` 函数供其他模块使用
- **package.json 脚本**：
...
- **依赖**：
  - `../../data-source` - 导入 AppDataSource
  - `../../entities/dish.entity` - 导入 Dish 实体和 Nutrition 接口
- **使用场景**：
  - 初始化开发环境的测试数据
  - 为新部署的数据库填充初始数据
  - 为测试提供示例数据
- **注意事项**：
  - 种子数据不关联用户（`userId: null`）
  - 营养成分数据基于常见菜品的平均值
  - 可以根据实际需求扩展更多菜品数据
  - 建议在运行迁移后执行种子脚本
...
#### `utils/`
- **作用**：存放工具函数和辅助函数，供各个模块使用
- **位置**：`src/utils/`
- **包含内容**：
  - `nutrition-calculator.ts` - 营养成分计算工具模块

#### `utils/nutrition-calculator.ts`
- **作用**：营养成分计算工具模块，根据菜品名称和分量估算营养成分
- **功能**：
  - 提供 `estimateNutrition` 函数，根据菜品名称和分量估算营养成分
  - 提供 `getDefaultNutrition` 函数，获取默认营养成分值
  - 实现基于关键词匹配的规则系统
  - 实现根据分量缩放营养成分的功能
- **核心函数**：
  - `estimateNutrition(dishName: string, portion: number = 200)`: 估算营养成分
    - 参数：
    ...
    - 功能流程：
      ...
      6. 返回缩放后的营养成分
  - `getDefaultNutrition(portion: number = 200)`: 获取默认营养成分值
    - 参数：`portion: number` - 分量（克），默认 200g
    - 返回值：`Nutrition` - 默认营养成分
    - 用途：用于未知菜品或作为基础值
- **营养成分规则系统**：
  - 基于关键词匹配的规则系统，包含多种常见食材和菜品类型
  - **高蛋白类**（肉类、蛋类）：
...
- **技术特点**：
  ...
  - 函数为纯函数，无副作用，便于测试
- **使用场景**：
  - 在用户手动录入菜品时，提供营养成分估算建议
  - 在图像识别功能中，根据识别出的菜品名称估算营养成分
  - 在饮食记录功能中，快速估算菜品的营养成分
- **依赖**：
  - `../entities/dish.entity` - 导入 `Nutrition` 接口
- **注意事项**：
  - 此工具函数基于简单规则和固定值，不是精确的营养成分数据库
  - 估算结果仅供参考，实际营养成分可能因烹饪方法、食材质量等因素而有所不同
  - 未来可以扩展为使用真实的营养成分数据库或调用第三方 API
